...
| └ `project-facts.md` | 用户确凿断言的项目事实：入口、配置、术语锚点、代码约定（每个事实名一条现行值） | 用户在对话中用肯定语气断言，且确认登记后 |
...
**确凿性门槛**：只有用户**用肯定语气断言**的事实才进真相层——疑问句（"是不是会校验 BOM？"）、"我记得好像"、讨论中的倾向一律不记。AI 自己读代码得出的结论也不算：代码告诉你系统做了什么，不告诉你业务要什么，它只能先落"未验证"，等用户点头才登记。宁可漏记，不可误记。
