...
| 2026-08-10 | `memory-model` | v0.2.2：maintain 来源链接补全（F-006） | maintain 的 --from-plan/--from-conversation 接受裸记录名、记忆根相对路径、项目根相对路径三种写法，CLI 统一归一为记忆根相对完整路径：剥记忆根前缀后，值内不含 / 时补默认目录段（plan-records/ 或 conversation-records/），无 .md 时补后缀；以 ../ 开头的视为调用方显式指定的记忆根外引用，原样保留不做任何补全。 | [2026-08-10_001_v0.2.2：maintain-来源链接补全（f-006）.md](2026-08-10_001_v0.2.2：maintain-来源链接补全（f-006）.md) |
| 2026-08-23 | `memory` | v0.3.0：确凿事实登记（F-007） | 用户在对话中确凿断言的项目事实，经用户确认后登记进 business-context/project-facts.md，事实名为稳定 key，同一时刻只有一条现行值。 | [2026-08-23_001_v0.3.0：确凿事实登记（f-007）.md](2026-08-23_001_v0.3.0：确凿事实登记（f-007）.md) |
